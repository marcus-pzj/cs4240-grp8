    ...
    */
    ...
    Vector3 Refract(float RI1, float RI2, Vector3 surfNorm, Vector3 incident)
    {
        ...
        Vector3 refractedRay =
            ...
            RI1 /
            RI2 *
            Vector3.Cross(surfNorm, Vector3.Cross(-surfNorm, incident)) -
            surfNorm *
            Mathf
                .Sqrt(1 -
                Vector3
                    .Dot(Vector3.Cross(surfNorm, incident) *
                    (RI1 / RI2 * RI1 / RI2),
                    Vector3.Cross(surfNorm, incident)))
            ).normalized;
    ...
    void Update()
    {
        ...
        {
            CalcLaserLine(laser.transform.position +
            laser.transform.forward * 0.6f,
        ...
        */
        if (lines.Count < 30)
        {
            ...
            if (intersect)
            {
                ...
                Transform parentObstacleTransform =
                    hit.transform.gameObject.transform.root;
                GameObject parentObstacleGO =
                    parentObstacleTransform.gameObject;
                ...
                float obstacleRI =
                    parentObstacleGO.GetComponent<Obstacle>().RefractiveIndex;
                bool isMirror =
                    parentObstacleGO.GetComponent<Obstacle>().isMirror;
                bool isOpaque =
                    parentObstacleGO.GetComponent<Obstacle>().isOpaque;
                ...
                */
                if (
                    CheckDirection(parentObstacleTransform.position,
                    hitPosition,
                    hit.normal)
                )
                {
                ...
                if (isOpaque)
                {
                    // If it is opaque. No reflection. Only show angle
                    DrawNormal(currLine,
                    hitPosition,
                    hitPosition + 0.5f * hit.normal);

                    DrawAngle(currLine,
                    hitPosition + 1.5f * hit.normal,
                    GetAngle(direction, hit.normal)); // for incident ray
                }
                else if (
                    isMirror ||
                    Refract(incidentRI, refractedRI, hit.normal, direction) ==
                    Vector3.zero
                )
                {
                    CalcLaserLine(hitPosition,
                    Vector3.Reflect(direction, hit.normal)); // Reflection
                    DrawNormal(currLine,
                    hitPosition,
                    hitPosition + 0.5f * hit.normal);
                    DrawAngle(currLine,
                    hitPosition + 1.0f * hit.normal,
                    GetAngle(direction, hit.normal)); // for incident ray
                }
                else
                {
                    ...
                    CalcLaserLine(hitPosition + 0.1f * direction,
                    Refract(incidentRI, refractedRI, hit.normal, direction)); // Refraction
                    DrawNormal(currLine,
                    hitPosition - 0.5f * hit.normal,
                    hitPosition + 0.5f * hit.normal);
                    DrawAngle(currLine,
                    ...
                    GetAngle(direction, hit.normal)); // for incident ray
                    DrawAngle(currLine,
                    hitPosition - hit.normal,
                    GetAngle(Refract(incidentRI,
                    refractedRI,
                    hit.normal,
                    direction),
                    hit.normal)); // for refracted ray
                }
            }
        }
    }
    ...
    void DrawLine(Vector3 startPosition, Vector3 finishPosition)
    {
        ...
        VolumetricLines.VolumetricLineBehavior line =
            go.GetComponent<VolumetricLines.VolumetricLineBehavior>();
        lines.Add (go);
    ...
    void DrawNormal(
        GameObject parentRay,
        Vector3 startPosition,
        Vector3 finishPosition
    )
    {
        GameObject go =
            Instantiate(NormalPrefab, Vector3.zero, Quaternion.identity);
        VolumetricLines.VolumetricLineBehavior line =
            go.GetComponent<VolumetricLines.VolumetricLineBehavior>();
        line.StartPos = startPosition;
        line.EndPos = finishPosition;
